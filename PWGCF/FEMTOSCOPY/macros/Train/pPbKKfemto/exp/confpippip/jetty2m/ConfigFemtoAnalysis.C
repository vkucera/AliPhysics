...
AliFemtoManager* ConfigFemtoAnalysis() {
  ...
  const int cMu=2; //numbwr of multiplicity bins
  int runmults[cMu] = {1, 1};
  double multbins[cMu+1] = {0.01, 400, 900};//LEGO Train
  ...
  const char *spart[2] = {"pip", "pim"};
  const char *sKK[2] = {"pippip", "pimpim"};//CF names
  const int zar[2]={+1,-1};
  ...
  for (int m=0; m<cMu; m++) {
	...
	pid_[ix] = new AliFemtoESDTrackCut();
	...
	pid_[ix]->SetMass(m_pi);
	pid_[ix]->SetMostProbablePion();
	...
	*/
	...
	mdca[0][ix] = new AliFemtoCutMonitorParticleYPt(Form("_Pass_%s_M%i",spart[zz],m),m_pi);
	mdca[1][ix] = new AliFemtoCutMonitorParticleYPt(Form("_Fail_%s_M%i",spart[zz],m),m_pi);
	pid_[ix]->AddCutMonitor(mdca[0][ix],mdca[1][ix]);

	mpid[0][ix] = new AliFemtoCutMonitorParticlePID(Form("_Pass_%s_M%i", spart[zz],m),0);//0 - pion time, 1 - kaon time
	mpid[1][ix]  = new AliFemtoCutMonitorParticlePID(Form("_Fail_%s_M%i",spart[zz],m),0);
